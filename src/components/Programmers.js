...
export default function Programmers() {
  ...
  const [featured, getFeatured] = useState(null);
  ...
  const getNameOfFeatured = () => {
    ...
    return pioneers.find(e => e.id === featured).name;
  };
          ...
          pioneers.map(dev =>
            <div className='programmer' key={dev.id}>
              {dev.name} <button onClick={() => { 
                featured = dev.id/* in here set the featured id to be dev.id */ }}>Feature</button>
            </div>
          )
        }
      </div>
